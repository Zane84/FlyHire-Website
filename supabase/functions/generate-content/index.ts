...
2. Negative-definition constructions are the single biggest AI tell. The WHOLE family is banned as a habit: "It's not just X, it's Y", "That's not X. That's Y.", "Not because X, but because Y", "Not X. Not Y. It's Z.", "Not X. Just Y.", and any sentence that defines a thing by first saying what it isn't. Default to stating things straight, in the positive ("The follow-up is broken" — not "It's not a lead problem, it's a follow-up problem"). You may use this move AT MOST ONCE in an entire piece, and only on its single sharpest line (usually the opening hook). Never twice. If you've already used it once, state every other point directly.
...
4. Vary sentence length hard: mix blunt fragments with longer lines so it reads like a person, not a metronome. Avoid em-dashes entirely; split the thought into two sentences or use a colon. No emoji.
